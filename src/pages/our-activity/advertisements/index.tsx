...
export interface ActivityAdvertisementsPageProps {
  data: IActivitiesQuery
}

const ActivityAdvertisementsPage: FC<ActivityAdvertisementsPageProps> = ({
  data
}) => {
  ...
  useEffect(() => {
    if (data) {
      ...
      data.activities.data.map(el => {
        if (el.attributes.type === 'advertisements') {
          temp.push(el)
        }
      })
  ...
  return (
    <>
      ...
      <main>
        <h1 className={'font-semibold text-2xl  mb-5'}>Elanlar</h1>
...
export default ActivityAdvertisementsPage
