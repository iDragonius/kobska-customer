...
interface SpecialMembersPageProps {
  data: IMemberQuery
}

const SpecialMembersPage: FC<SpecialMembersPageProps> = ({ data }) => {
  ...
  return (
    <>
      ...
      <main>
        ...
        >
          <h1 className={'text-2xl font-semibold'}>{t('special_member')}</h1>
        </div>
        ...
        >
          {membersData.data.map(el => {
            if (
              el.attributes.member_type?.data?.attributes?.type === 'Xüsusi üzv'
            )
...
export default SpecialMembersPage
